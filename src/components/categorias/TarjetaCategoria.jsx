...
const TarjetaCategoria = ({ categorias, abrirModalEdicion, abrirModalEliminacion }) => {
    const [cargandoInterno, setCargandoInterno] = useState(true);
    const [idTarjetaActiva, setIdTarjetaActiva] = useState(null);

    useEffect(() => {
        setCargandoInterno(!(categorias && categorias.length > 0));
    }, [categorias]);
            ...
            {categorias.map((categoria) => {
                ...
                return (
                    <Card
                        ...
                        tabIndex={0}
                        onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && alternarTarjetaActiva(categoria.id_categoria)}
                    >
                        <Card.Body className={`p-2 tarjeta-categoria-cuerpo ${tarjetaActiva ? "tarjeta-categoria-cuerpo-activo" : "tarjeta-categoria-cuerpo-inactivo"}`}>
                            <Row className="align-items-center gx-3">
                                ...
                                <Col xs={7} className="text-start">
                                    <div className="fw-semibold text-truncate">{categoria.nombre_categoria}</div>
                                    <div className="small text-muted text-truncate">{categoria.descripcion_categoria}</div>
                                </Col>
                                <Col xs={3} className="text-end">
                                    <div className="fw-semibold small text-success">Activa</div>
                                </Col>
                            </Row>
                        </Card.Body>
                        ...
                        {tarjetaActiva && (
                            <div className="tarjeta-categoria-capa" onClick={(e) => { e.stopPropagation(); setIdTarjetaActiva(null); }}>
                                <div className="d-flex gap-2 tarjeta-categoria-botones-capa" onClick={(e) => e.stopPropagation()}>
                                    <Button variant="outline-warning" size="sm" onClick={() => { abrirModalEdicion(categoria); setIdTarjetaActiva(null); }}>
                                        <i className="bi bi-pencil"></i>
                                    </Button>
                                    <Button variant="outline-danger" size="sm" onClick={() => { abrirModalEliminacion(categoria); setIdTarjetaActiva(null); }}>
